...
/*int Arduino_UNOWIFIR4_SSE::exportPubKey(const unsigned char* inder, int inlen, unsigned char* outder, int outlen)
{
  int ret = 1;
  mbedtls_pk_context key;
  unsigned char tmp[128] = {0};
  ...
  if ((ret = mbedtls_pk_parse_key(&key, inder, inlen, NULL, 0)) != 0) {
    DEBUG_ERROR(" failed\n  !  mbedtls_pk_parse_key returned -0x%04x", (unsigned int) -ret);
    goto exit;
  }
  ...
  memcpy(outder, &tmp[sizeof(tmp)- ret], ret);
...
int Arduino_UNOWIFIR4_SSE::verify(const unsigned char* der, int len, const unsigned char* sha256, unsigned char* signature)
{
  ...
  log_e("compressed signature %d", 64);
  log_buf_v((const uint8_t *)signature, 64);

  /* Move to external conversion function */
  /* Convert signature {r,s} values to asn.1 */
  size_t retSize;
  ...
  log_e("rebuild signature %d", retSize);
